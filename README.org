...
***** if input= matches regex ([0-9]*-[0-9]*) grab table of contents from that page range synopsize and validate with user
...
** Narration Input Toggle: 
*** todo optionally use ? https://github.com/coqui-ai/TTS ? to generate voice to narrate gpt response&queries to user

** VoiceDiction Output Toggle: 
*** todo optionally use ?talon? to allow voice input?
